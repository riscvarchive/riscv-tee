...
.. Adding an *M-mode-only* or a *locked* *Shared-Region* rule with executable privileges is not possible and such ``pmpcfg`` writes are ignored, leaving ``pmpcfg`` unchanged. This restriction can be temporarily lifted e.g. during the boot process, by setting ``mseccfg.RLB``.
