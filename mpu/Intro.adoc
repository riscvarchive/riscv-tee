...
* *PMP Rule*: The contents of a pmpcfg register and its associated pmpaddr register(s), that encode a valid protected physical memory region, where ``pmpcfg[i].A != OFF``, and in case ``pmpcfg[i].A == TOR``, ``pmpaddr[i-1] < pmpaddr[i]``.
...
However, there are no such mechanisms available on Machine mode in the current Privilege Spec. If we want a PMP rule to be *enforced* only on non-Machine modes and *denied* on Machine mode, so that we can only allow access to a memory region by less-privileged modes, it's not possible. We can only have a *locked* rule that will be *enforced* on all modes, or a rule that will be *enforced* on non-Machine modes and be *ignored* by Machine mode. So on any physical memory region not protected with a Locked rule, Machine mode has unlimited access, including the ability to execute it.
